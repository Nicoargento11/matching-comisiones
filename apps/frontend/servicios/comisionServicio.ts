import { api } from "./api";
import { ComisionSinColor, Horario, Evento } from "@/tipos";
import { agregarColor } from "@/lib/colores";

export const comisionServicio = {
  obtenerTodas: (token?: string) =>
    api.get<ComisionSinColor[]>("/comisiones", token).then((cs) => cs.map(agregarColor)),

  obtenerPorId: (id: number, token?: string) =>
    api.get<ComisionSinColor>(`/comisiones/${id}`, token).then(agregarColor),

  agregarEstudiante: (idComision: number, idUsuario: number, token?: string) =>
    api.post<void>(
      `/comisiones/${idComision}/estudiantes`,
  ...
  darBajaEstudiante: (idComision: number, idUsuario: number, token?: string) =>
    api.delete<void>(
      `/comisiones/${idComision}/estudiantes/${idUsuario}`,
      token,
    ),
  ...
  reactivarHorario: (idComision: number, idHorario: number, token?: string) =>
    api.patch<Horario>(
      `/comisiones/${idComision}/horarios/${idHorario}/reactivar`,
      {},
      token,
    ),
  ...
  modificarEvento: (
    ...
    api.patch<Evento>(
      `/comisiones/${idComision}/eventos/${idEvento}`,
      datos,
      token,
    ),
  ...
  reactivarEvento: (idComision: number, idEvento: number, token?: string) =>
    api.patch<Evento>(
      `/comisiones/${idComision}/eventos/${idEvento}/reactivar`,
      {},
      token,
    ),
};
